...
cp -av "$DIR/colorbars.jpg" /mnt/var/www/samygo-web-api/
cp -av "$DIR/cgi-bin/samygo-web-api.cgi" /mnt/var/www/samygo-web-api/

echo "Installing cjpeg dependency to /mnt"
cp -av "$DIR/cjpeg/cjpeg" /mnt/bin/
cp -av "$DIR/cjpeg/libjpeg.so.62" /mnt/usr/lib/

echo "Installing startup script to /mnt/etc/init.d/"
cp -av "$DIR/init.d/99_99_samygo_web_api.init" /mnt/etc/init.d/
